...
Function.prototype.appendFunction ||
  (Function.prototype.appendFunction = function (a) {
    var b = this.name;
    "function" != typeof window[b]
      ? console.warn("function " + b + " is not exist")
      : (window[b] = (function (c) {
          return function () {
...
function onLoginSubmitted() {
  ...
  1 > document.login.username.value.length
    ? ((a.innerText = "يجب ادخال قيمة صحيحة!"),
      showErrorPopup(),
      (animating = !1))
    : ((a.innerText = ""),
...
function showParentElement(a, b) {
  ...
  setTimeout(function () {
    $status.classList.contains("active") && b !== $status
      ? hideStatusPage()
      : hideLoginPage();
    animating = !1;
...
function initHotspotEvents(a, b) {
  ...
  a.addEventListener("click", function (f) {
    animating ||
      ...
      "status" === b.id
        ? onLoginSubmitted()
        : ("app-store" === b.id && loadScript("js/store.min.js"),
...
function toArabicTime(a) {
  ...
  if ("" === a) return "";
  0 <= a.indexOf("d") &&
    ((b = a.split("d")), (c = "" !== b[0] ? b[0] + " يوم " : ""), (b = b[1]));
  0 <= a.indexOf("h") &&
    ((b = b.split("h")), (d = "" !== b[0] ? b[0] + " ساعة " : ""), (b = b[1]));
  0 <= a.indexOf("m") &&
    ((b = b.split("m")), (e = "" !== b[0] ? b[0] + " دقيقة " : ""), (b = b[1]));
  0 <= a.indexOf("s") &&
    ((a = b.split("s")), (f = "" !== a[0] ? a[0] + " ثانية " : ""));
  return c + d + e + f;
}
function toArabicBytes(a) {
  return 1024 > a
    ? a + " بايت "
    : 1048576 > a
    ? Math.round(a / 1024) + " كيلوبايت "
    : 1073741824 > a
    ? Math.round(a / 1048576) + " ميجابايت "
  ...
  if (a.logged_in)
    for (var b in a) {
      ...
      if ("string" == typeof c && "logged_in" !== b) {
        ...
        null != d &&
          ("" === c
            ? ((c = document.querySelector("[".concat(b, "]"))),
              null != c && (c.style.display = "none"))
            : "username" === b
            ? (d.innerText = hideHalfCard(c))
            : b.includes("bytes")
            ? (d.innerText = toArabicBytes(c))
            : b.includes("time")
            ? (d.innerText = toArabicTime(c))
            : (d.innerText = c));
      }
    }
  else loggedIn = !1;
  try {
    a.username.includes(a.mac.split("%3A").join(":")) &&
      ((b = "كرت مجاني من خدمة اجمع واربح!"),
      a.username.includes("T-") && (b = "انت الان تستخدم خدمة سلفني"),
      (document.getElementById("status-title").innerText = b));
  } catch (e) {}
}
    ...
    b.onreadystatechange = function () {
      if (4 === this.readyState) {
        ...
        try {
          ...
          "/status?var=callBack" === a &&
            ((loggedIn = 0), hideStatusPage(), showLoginPage(), cookieLogin());
        }
      }
    };
    b.send();
...
function userLogin() {
  var a = document.login.querySelector("input[username-field]").value,
    b,
    c =
      null != (b = document.login.querySelector("input[password-field]"))
        ? b
        : "";
  "" !== c && (c = c.value);
...
function startStatusQuery() {
  if (!intervalStatus && loggedIn) {
    ...
    var a = setInterval(function () {
      loggedIn
        ? getRequest("/status?var=callBack")
        : ((intervalStatus = !1), clearInterval(a));
    }, 1e3);
  }
}
